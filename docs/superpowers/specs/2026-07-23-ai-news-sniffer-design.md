...
每个适配器只负责从一种来源获取候选条目，并转换成统一的原始新闻结构。首版采用分层白名单采集，配置 35 个官方、研究与开源、可信媒体和社区发现来源；RSS、Atom、公开 API 和 GitHub Releases 优先，只对白名单中的少数重要官网使用 HTML 解析。

来源清单、单来源开关、来源组开关、`light`/`balanced`/`full` 运行模式、类别和基础权重均写入配置，不硬编码在业务逻辑中。AI 不得自行启用新来源；发现的新域名只能进入待审核清单。首版不接入付费搜索 API 或浏览器自动化，但保留新的来源适配器接口。

完整来源目录、运行模式、Token 预算、事实确认和健康策略见 `docs/superpowers/specs/2026-07-23-ai-news-source-strategy-design.md`。
...
事件同时标记为官方或原文确认、独立媒体交叉确认、未确认三种状态之一。未确认事件保留在运行记录中，但不能进入正式日报。
...
- 同一主要来源每天最多 2 条，同一类别每天最多 3 条。
- 同一事件只能占一个主条目。
- 社区来源不能直接成为主要来源，未确认事件不能发布。
...
- `config/app.yaml` 的 `editorial` 段：模型候选数量、单条摘要字符数和总提示字符预算。
- `config/sources.yaml`：35 个来源、来源组、分类、权重、单来源开关和 `light`/`balanced`/`full` 模式。
...
单个来源超时、格式异常或无法访问时记录错误并继续。连续失败 3 次时标记来源降级，连续失败 7 次时自动暂停并发送维护提醒。候选新闻不足时生成精简日报，并在运行信息中说明有效来源数量。禁止自动访问、必须执行 JavaScript 或无法稳定解析的来源应暂停，不绕过访问限制。
...
- 来源解析和异常输入。
- 来源、来源组、运行模式和手动覆盖的开关优先级。
- 模型候选数量、单条摘要和总提示字符预算。
- 事实确认状态、独立来源判断和社区上游原文验证。
- HTML 白名单解析和来源健康状态。
- URL 规范化。
- 标题近似去重。
- 评分计算和权重配置。
...
3. 35 个初始来源均存在于配置中，任一来源和来源组可以关闭，三种运行模式能启用正确来源。
4. 固定样本中的重复事件能被合并。
5. 可使用 DeepSeek 生成结构化中文摘要，并能通过配置切换模型端点；模型输入受到候选数量和字符预算硬限制。
...
13. AI 不能发布候选池外来源或未确认事件，新来源只能进入待审核清单。
